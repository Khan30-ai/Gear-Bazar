...
import { Pencil, KeyRound, LogOut, Check, X } from "lucide-react";
...
const FIELD_CONFIG = [
    ...
    { key: "gstNumber", label: "GST Number", mono: true },
...
export default function SellerProfile() {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await updateMySellerProfile(draft);
                ...
                <div className="rounded-sm border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md">
                    ...
                    <div className="flex flex-col gap-4 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:gap-8 sm:p-10">
                        <div className="flex items-center gap-4 sm:gap-8">
                            {/* Avatar */}
                            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-sm bg-orange-600 text-base font-bold tracking-tight text-white shadow-sm transition-all duration-300 hover:scale-[1.01] sm:h-24 sm:w-24 sm:text-3xl">
                                {initials}
                            </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 sm:mt-0 sm:flex-1 sm:justify-end sm:gap-3">
                            <span
                                className={`inline-flex items-center gap-1.5 rounded-sm border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] sm:text-[11px] ${statusStyles[seller.status]}`}
                            >
                                <span className={`h-1.5 w-1.5 rounded-full ${statusDot[seller.status]}`} />
                                {statusLabel}
                            </span>
